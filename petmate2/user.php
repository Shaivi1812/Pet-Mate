...
<!-- <form style="display:none" id=dialog action="changepets.php" method="post" target="post_pets_frame" enctype="multipart/form-data">
	...
	<input style="display: none;" type="text" name="posted_on" id="posted_on">
	<input style="display: none;" type="text" name="accid" id="accid">
	<br>
</form> -->

<form style="display: none" id="acc_set_form" action="changeacc2.php" method="POST" target="post_acc_frame">
	<p id=dmsg2>Only Fill The Ones To Be Updated</p>
	<table>
		<tr><td><label class="mx-1" for="fname">FIRST NAME: </label></td> <td><input type="text" name="fname" id="fname"></td></tr>
		<tr><td><label class="mx-1" for="lname">LAST NAME:</label></td><td><input type="text" name="lname" id="lname"></td></tr>
		...
		<!-- <button class="ok_acc_btn" id="ok_acc_button" class="m-2" value="okay" name="ok_acc_button">OKAY</button> -->
</form>

<div id=pets_info class="container flex-row flex-wrap mt-3">
</div>	
<form style="visibility: hidden" id="logout_form" target="_top" method="GET" action="login.php"></form>
<!-- <form style="visibility: hidden" id="reload_page" target="_top" method="GET" action="user.php">
	<input type="text" name="userid" id="reload_page_userid">
</form> -->

<!-- <form style="visibility: hidden" id="delete_acc_form" target="post_acc_del_frame" method="POST" action="deleteacc.php" >
	<input type="text" name="delaccid" id="delaccid">
</form>
 -->
<!-- <iframe style="display: none" src="" name="post_pets_frame"></iframe> -->
<iframe style="display: none" src="" name="post_acc_frame"></iframe>
<!-- <iframe style="display: none" src="" name="post_acc_del_frame"></iframe> -->
...
// $("#delaccid").val(acc_id);
$("#pets_info").load("pets_info2.php");
$("#acc_name").load("nameplate2.php");
// $( "#dialog" ).dialog({
//     autoOpen: false,
//     position: {
//       my: "center",
...
//                 $( this ).dialog( "close");
//                 $("#dialog").ajaxSubmit({success:function(){$("#pets_info").load("pets_info.php?accid="+acc_id);}});
    ...
    buttons: [
        {
            ...
            click: function() {
                ...
                $("#acc_set_form").ajaxSubmit({success:function(){
			    	$("#pets_info").load("accountsettings2.php");
			    	$("#acc_name").load("nameplate2.php");
...
// $("#pets_info").on("click",".ok_acc_btn",function(){
//     $("#acc_set_form").ajaxSubmit({success:function(){
//     	$("#pets_info").load("accountsettings.php?accid="+acc_id);
//     	$("#acc_name").load("nameplate.php?accid="+acc_id);
//     }});
...
if(isset($_POST['deluser'])){
	...
	echo "<script type='text/javascript'>console.log('delete request user_id=$user_id');</script>\n";
	...
	if(!$conn){
		...
		$sql = "DELETE FROM petmate_users where user_id=$user_id";
		...
		if(!$result){
			...
			echo "<script type='text/javascript'>alert('YOUR ACCOUNT WITH ID=$user_id WAS DELETED');</script>\n";
			session_destroy();
		}
	}
}
